...
export default function ToolForm({ toolId, initialData }: ToolFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    generic_description: initialData?.generic_description || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataToSend.append('generic_description', formData.generic_description)
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          onChange={handleChange}
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          placeholder="tool-slug-url-friendly"
        />
        <p className="mt-1 text-xs text-gray-500">URL-friendly identifier (e.g., "my-awesome-tool")</p>
      </div>

      {/* Generic Description */}
      <div>
        <label htmlFor="generic_description" className="block text-sm font-medium text-gray-700 mb-2">
          Generic Description
        </label>
        <textarea
